...
if command -v npm &> /dev/null; then
    ...
    npm run test:frontend -- --watch=false --browsers=ChromeHeadless 2>/dev/null
    frontend_result=$?
    ...
    if npm run test:frontend -- --watch=false --browsers=ChromeHeadless 2>&1 | grep -q "Tests passed!"; then
