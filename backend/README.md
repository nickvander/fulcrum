...
This directory contains the Python FastAPI application that serves as the
backend for the Fulcrum platform.
...
- Interfacing with AI services for tasks like semantic search and content
  generation.
...
1.  **Environment Variables:**
    ...
    - Review and update the variables in `.env` if necessary (the defaults are
...
2.  **Build and Run Containers:**
    ...
    - Run the following command:
      ...
      ```
    - This will build the Docker image for the backend, pull the required
      database and Redis images, and start all the services.
...
This project uses Alembic to manage database schema changes. A wrapper script,
`migrate.sh`, is provided to simplify the process of running migrations within
the Docker container.

### Generating a New Migration

After making changes to the SQLAlchemy models in `src/models/`, you need to
generate a new migration script. Run the following command from the project
root:
