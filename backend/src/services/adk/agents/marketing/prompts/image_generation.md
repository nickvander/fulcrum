...
3.  **Content Context**: The text of the post that was just written (so the
    image matches).
4.  **Visual Style**: (Optional) e.g. "Minimalist", "Lifestyle", "Studio".

## Instructions

Create a detailed image generation prompt that would result in a high-quality,
professional marketing image.

- Focus on lighting, composition, and texture.
- Ensure the product is the hero.
- Include technical keywords (e.g. "8k resolution", "photorealistic", "depth of
  field").
