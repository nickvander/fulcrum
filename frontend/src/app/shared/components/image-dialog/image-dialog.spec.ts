...
describe('ImageDialogComponent', () => {
  ...
  beforeEach(async () => {
    productServiceMock = jasmine.createSpyObj('ProductService', ['updateProductImage']);
  ...
  describe('save functionality', () => {
    ...
    it('should call updateProductImage when form is valid and save is clicked', () => {
      productServiceMock.updateProductImage.and.returnValue(of({ ...mockImage, title: 'Updated Title', description: 'Updated Description' }));

      component.onSave();

      expect(productServiceMock.updateProductImage).toHaveBeenCalledWith(
        1,
        1,
    ...
    it('should show success notification and close dialog on successful save', () => {
      const updatedImage = { ...mockImage, title: 'Updated Title', description: 'Updated Description' };
      productServiceMock.updateProductImage.and.returnValue(of(updatedImage));
    ...
    it('should show error notification when saving fails', (done) => {
      const errorResponse = 'Error occurred';
      productServiceMock.updateProductImage.and.returnValue(throwError(() => errorResponse));

      component.onSave();

      // Since this is an asynchronous operation, we need to wait for the error to be processed
      setTimeout(() => {
