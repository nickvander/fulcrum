...
export interface ProductFormInitializerConfig {
  shouldFailCustomFields?: boolean;
  shouldFailProduct?: boolean;
  customFieldsError?: any;
  productError?: any;
}
...
export class ProductFormInitializerServiceTestHelper {
  private config: ProductFormInitializerConfig = {};
  ...
  setConfig(config: ProductFormInitializerConfig): void {
    this.config = config;
  }
  ...
  initializeForm(isEditMode: boolean, productId: number | null): Observable<ProductFormInitializationData> {
    ...
    const customFields$ = this.config.shouldFailCustomFields 
      ? throwError(() => this.config.customFieldsError || new Error('Custom fields fetch failed'))
    ...
    if (isEditMode && productId) {
      product$ = this.config.shouldFailProduct
        ? throwError(() => this.config.productError || new Error('Product fetch failed'))
