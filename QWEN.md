...
8.  **Git Hooks for Quality Assurance:** The project uses git hooks to ensure
    code quality before commits and pushes:
    - **pre-commit hook:** Runs fast backend tests and linter to catch issues
      early.
    - **pre-push hook:** Runs the full CI test suite (backend, frontend, and
      linting) to ensure all code passes comprehensive tests before being pushed
      to the repository.
    - Both hooks are automatically installed in the `.git/hooks/` directory and
