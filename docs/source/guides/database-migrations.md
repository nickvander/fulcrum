...
Alembic is configured in the `backend/alembic.ini` file. It is set up to read the database connection string from the `DATABASE_URL` environment variable, which is passed to the container by Docker Compose. The migration scripts themselves are located in `backend/alembic/versions/`.
...
**You should always use this script to perform migration tasks.** It ensures that the `alembic` command is executed with the correct environment and context inside the `backend` container.
