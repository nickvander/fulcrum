...
5.  **CI Linter Failure:** The linter fails because it cannot fix unused imports in a migration file (`eca3f3875308_add_custom_field_models.py`) due to a `Permission denied` error. The file is owned by `root` because it was generated inside the Docker container.
...
-   **CI Failures:** We fixed a `pytest: not found` error by ensuring the test command runs inside the activated virtual environment.
...
### 2. **Fix Linter Permission Error**
-   **Hypothesis:** The permission error on the migration file is because the `alembic` command was run as `root`.
-   **Action:**
    1.  Modify the `docker compose exec` command used for generating migrations to run as the `appuser`. For example: `docker compose exec -u appuser backend alembic revision ...`.
    2.  This will ensure that any new auto-generated files are created with the correct ownership, preventing future linter and permission issues. We will need to regenerate the problematic migration file.

### 3. **Diagnose and Fix Frontend Proxy & Backend Stability**
...
### 4. **Diagnose and Fix Photo Ingestion**
