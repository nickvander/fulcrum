...
Update the product image workflow to allow temporary image deletion and improve
overall UX by implementing proper save/revert functionality and custom field
support.

## Current Issues

1. When editing a product and deleting an image, the user is taken straight back
   to the product screen instead of allowing temporary changes
...
- Only after clicking "Save" should the image be permanently deleted and user
...
- When user clicks "Cancel" or "Back" without saving, discard temporary changes
  and return to product list
...
1. Modify image deletion logic to mark images as "deleted" temporarily instead
   of immediately removing
2. Add a mechanism to track which images are marked for deletion
3. Update the save functionality to process "marked for deletion" images
   permanently
...
1. Update the save button behavior to navigate to product list after successful
   save
...
- Update API calls to handle batch operations (save product + delete images +
  custom fields)
